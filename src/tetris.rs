use std::{collections::HashSet, vec, mem};

use crate::shape::{Shape, Pos};
...
impl Tetris{
    ...
    pub fn is_colliging(&self, shape: &Shape) -> bool {
        self.fixed_shapes
        .iter()
    ...
    pub fn tick(&mut self) {
        ...
        if self.is_out_of_bounds(&translated_current_shape) 
            || self.is_colliging(&translated_current_shape) 
        {
            // Move current shape to fixed shape
            ...
            if self.is_colliging(&self.current_shape) {
                self.lost = true
            }
        } else {
    ...
    pub fn rotate(&mut self) {
        ...
        if !self.is_out_of_bounds(&rotated_current_shape) 
            && !self.is_colliging(&rotated_current_shape)
        {
